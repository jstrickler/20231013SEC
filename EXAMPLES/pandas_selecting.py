...
df = pd.DataFrame(values, index=index, columns=columns)  # create dataframe with data, row labels, and column labels
...
print_header("df['b':'b']")
print(df['b':'b'], '\n')  # select row 'b' only using slice of row labels (returns dataframe)
